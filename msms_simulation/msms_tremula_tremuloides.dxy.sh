...
$angsd -glf $OutDir/tremula.$file.glf.gz -nInd 48 -doSaf 1 -anc $anc -fai $ref -pest $OutDir/tremula.$file.sfs -out $OutDir/tremula.$file.rf
$angsd -glf $OutDir/tremuloides.$file.glf.gz -nInd 44 -doSaf 1 -anc $anc -fai $ref -pest $OutDir/tremuloides.$file.sfs -out $OutDir/tremuloides.$file.rf

gunzip -c $OutDir/tremula.$file.rf.saf.pos.gz > $OutDir/tremula.$file.rf.saf.pos
gunzip -c $OutDir/tremuloides.$file.rf.saf.pos.gz > $OutDir/tremuloides.$file.rf.saf.pos

awk 'FNR==NR {x[$1"_"$2]=NR; next} x[$1"_"$2] {print x[$1"_"$2]; print FNR > "/dev/stderr"}' $OutDir/tremula.$file.rf.saf.pos $OutDir/tremuloides.$file.rf.saf.pos >$OutDir/tremula.$file.rf.pos 2>$OutDir/tremuloides.$file.rf.pos

tremula_site=$(cat $OutDir/tremula.$file.rf.saf.pos | wc -l)
tremula_len=$(cat $OutDir/tremula.$file.rf.pos | wc -l )
echo $tremula_site
echo $tremula_len

tremuloides_site=$(cat $OutDir/tremuloides.$file.rf.saf.pos | wc -l)
tremuloides_len=$(cat $OutDir/tremuloides.$file.rf.pos | wc -l )

gunzip -c $OutDir/tremula.$file.saf.gz > $OutDir/tremula.$file.saf
gunzip -c $OutDir/tremuloides.$file.saf.gz > $OutDir/tremuloides.$file.saf

$GetSubSfs -infile $OutDir/tremula.$file.saf -posfile $OutDir/tremula.$file.rf.pos -nind 24 -nsites $tremula_site -len $tremula_len -outfile $OutDir/tremula.$file.fix.saf
$GetSubSfs -infile $OutDir/tremuloides.$file.saf -posfile $OutDir/tremuloides.$file.rf.saf.pos -nind 22 -nsites $tremuloides_site -len $tremuloides_len -outfile $OutDir/tremuloides.$file.fix.saf
