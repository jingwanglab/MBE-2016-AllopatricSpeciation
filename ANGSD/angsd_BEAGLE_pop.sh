#! /bin/bash -l


#set -e
#set -x

#SBATCH -A b2011141
#SBATCH -p core
#SBATCH -o angsd_beagle.out
#SBATCH -e angsd_beagle.err
#SBATCH -J angsd_beagle.job
#SBATCH -t 5-00:00:00
#SBATCH --mail-user jing.wang@emg.umu.se
#SBATCH --mail-type=ALL

angsd="/proj/b2011141/tools/angsd0.602/angsd"
bam_list_all="/proj/b2011141/nobackup/tremula_vs_tremuloides_paper/bwa_mem_alignment/bam/all/tremula_tremuloides.bam.list"
ref="/proj/b2011141/nobackup/reference/nisqV3/Ptrichocarpa_v3.0_210.fa"
OutDir="/proj/b2011141/nobackup/tremula_vs_tremuloides_paper/bwa_mem_alignment/ANGSD/beagle"
region="/proj/b2011141/nobackup/tremula_vs_tremuloides_paper/bwa_mem_alignment/bed/all/Chr"
chr=$region/all.filter.Chr$1.region


if [ ! -d "$OutDir" ]; then
mkdir -p $OutDir
fi


#first generate .beagle file
$angsd -bam $bam_list_all -GL 1 -doMajorMinor 1 -doMaf -1 -doGlf 2  -SNP_pval 1e-6 -out $OutDir/tremula_tremuloides.$1 -ref $ref -rf $chr -minMapQ 30 -minQ 20  


